...
import '../App.css'
import '../styles/ModificarMascota.css'
...
function ModificarMascota() {
  const { id } = useParams()
  const { formState, onInputChange, onResetForm, setFormState, name, type, description, skill1, skill2, skill3 } = useForm({
    name: '',
    type: '',
    description: '',
    skill1: '',
  ...
  const getAxios = () => {
    axios.get(`http://127.0.0.1:9000/api/getIdMascota/${id}`)
      .then(response => {
        setFormState(response.data.mascota);
  ...
  function handleSubmit(event) {
    ...
    axios.put(`http://127.0.0.1:9000/api/pets/${id}/edit`, mascota)
...
<button className={`btn ${boton}`} onClick={handleClassName}><img className={`img ${boton !== '' ? 'esconderse' : ''}`} src={'https://cdn-icons-png.flaticon.com/512/5305/5305380.png'}></img> Actualizar</button>
